...
<script id="gotei-0.1.0-notests">
  (function (global, factory) {
    ...
    function handleReactiveList(element, listExpr) {
      var _a;
      const template =
        (_a = element.querySelector("template")) == null
          ? void 0
          : _a.content.firstElementChild;
      if (!template) return;
        ...
        if (op) {
          switch (op) {
            ...
            case "unshift":
              ...
              for (
                let i = 0;
                i < newData.length - element.children.length;
                i++
              ) {
                const node = create(i);
      ...
      for (const { name, value } of element.attributes) {
        switch (true) {
          case name.startsWith(RX_ATTR_PREFIX):
            handleReactiveAttr(
              element,
              name.slice(RX_ATTR_PREFIX.length),
              value,
            );
            break;
